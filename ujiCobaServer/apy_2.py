...
def send_status_to_firebase(status, confidence):
    ...
    ref.set(status)
    # print(f"Status '{status}' dengan confidence {confidence:.2f} berhasil dikirim ke Firebase.")
...
def main():
    ...
    cap = cv2.VideoCapture(0)  # Ganti 0 dengan 1 jika menggunakan webcam eksternal
    ...
    while True:
        ...
        if preprocessed_eye_image is not None:
            ...
            print("Predictions (Eye):", predictions)  # Debug
            confidence_eye = float(np.max(predictions))
            predicted_class_eye = CLASSES[np.argmax(predictions)]
            print(f"Eye Prediction: {predicted_class_eye}, Confidence: {confidence_eye:.2f}")  # Debug
        ...
        if preprocessed_mouth_image is not None:
            ...
            print("Predictions (Mouth):", predictions)  # Debug
            confidence_mouth = float(np.max(predictions))
            predicted_class_mouth = CLASSES[np.argmax(predictions)]
            print(f"Mouth Prediction: {predicted_class_mouth}, Confidence: {confidence_mouth:.2f}")  # Debug
            current_time = time.time()
            ...
            if predicted_class_mouth == "yawn":
                if yawn_start_time is None:
                    ...
                    result = "yawn"
                    confidence = confidence_mouth
                    yawn_start_time = None
            else:
                yawn_start_time = None

        # Hanya kirim status jika bukan 'not_sleepy'
        if result != "not_sleepy":
        ...
        cv2.putText(frame, f"Prediction: {result} ({confidence:.2f})", (10, 30), 
